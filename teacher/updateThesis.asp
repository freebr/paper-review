...
ElseIf Not isMatched("[1-8]",opr) Then
	bError=True
	errdesc="操作无效。"
ElseIf submittype="unpass" And opr<=5 Or opr=4 Or opr=6 Or opr=8 Or opr=9 Then
...
'		tempfilename=toDateTime(append_time,1)&Int(Timer)&Int(Rnd()*999)&".docx"
...
'		detect_time=Now
'		Randomize
'		filename=toDateTime(detect_time,1)&Int(Timer)&Int(Rnd()*999)&".docx"
'		filepath=Server.MapPath("export")&"\"&filename
'		Set dag=New DetectAppGen
'		dag.Author=author
'		dag.StuNo=stuno
'		dag.TutorName=tutorname
'		dag.TutorInfo=tutorinfo
'		dag.Spec=speciality
'		dag.DegreeName=degreename
'		dag.Date=toDateTime(detect_time,1)
'		dag.Subject=subject
'		dag.EvalText=eval_text
'		dag.Mobile=mobile
'		dag.Email=email
'		dag.TutorMobile=tutormobile
'		dag.TutorMail=tutormail
'		bError=dag.generateApp(filepath)=0
'		Set dag=Nothing
		' 更新记录
