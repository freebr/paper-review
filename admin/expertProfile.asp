...
%><%=rs("TEACHERNO").Value%><%
	Else
%><input type="text" class="txt" name="teacherno" value="<%=HtmlEncode(rs("TEACHERNO").Value)%>" /><%
	End If
%></td>
...
<tr height="30">
	...
	<td bgcolor="white"><input type="text" class="txt" name="bankaccount" id="bankaccount" value="<%=HtmlEncode(rs("BANK_ACCOUNT").Value)%>" onkeyup="replNoNum(this)" onbeforepaste="replClipboardData('trimNoNum')" /></td>
