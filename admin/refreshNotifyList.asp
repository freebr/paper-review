...
' 导入不在通知列表中的待通知导师记录
sql="SELECT DISTINCT TUTOR_ID FROM ViewThesisInfo WHERE (TASK_PROGRESS IN (1,4,7,10) OR REVIEW_STATUS IN (1,5,8,11,14,15)) "&_
...
' 导入不在通知列表中的待通知专家记录
sql="SELECT DISTINCT REVIEWER1 FROM Dissertations WHERE REVIEWER1 IS NOT NULL AND REVIEWER_EVAL1 IS NULL "&_
		"UNION SELECT DISTINCT REVIEWER2 FROM Dissertations WHERE REVIEWER2 IS NOT NULL AND REVIEWER_EVAL2 IS NULL"
GetRecordSetNoLock conn,rs,sql,result
sql="INSERT INTO NotifyList (USER_ID,USER_TYPE) VALUES"
Do While Not rs.EOF
