...
sql="DECLARE @tmptbl TABLE(EXPERT_ID int,THESIS_SUBJECT nvarchar(200),REVNUM int);"&_
	"INSERT INTO @tmptbl SELECT REVIEWER1,MIN(THESIS_SUBJECT),COUNT(ID) FROM VIEW_TEST_THESIS_REVIEW_INFO WHERE ID IN ("&thesisID&") AND REVIEWER_EVAL1 IS NULL GROUP BY REVIEWER1 "&_
	"UNION ALL SELECT REVIEWER2,MIN(THESIS_SUBJECT),COUNT(ID) FROM VIEW_TEST_THESIS_REVIEW_INFO WHERE ID IN ("&thesisID&") AND REVIEWER_EVAL2 IS NULL GROUP BY REVIEWER2;"&_
	"SELECT EXPERT_ID,EXPERT_NAME,TEACHERNO,MOBILE,EMAIL,MIN(THESIS_SUBJECT) AS THESIS_SUBJECT,SUM(REVNUM) AS REVIEW_NUM FROM @tmptbl LEFT JOIN VIEW_TEST_THESIS_REVIEW_EXPERT_INFO ON EXPERT_ID=TEACHER_ID GROUP BY EXPERT_ID,EXPERT_NAME,TEACHERNO,MOBILE,EMAIL;"
