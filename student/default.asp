...
Function showStepInfo(stepDisplay,stepCounter,bHidden)
	If review_status<>stepDisplay And (stepDisplay=rsNotAgreeDetect Or stepDisplay=rsNotAgreeReview Or stepDisplay=rsModifyUnpassed) Then
		showStepInfo=False
		Exit Function
	Else
		showStepInfo=True
...
%><span style="color:dimgray">导师不同意您的论文进行检测，请修改论文并重新上传。<br/>送检意见：<%=toPlainString(rs("DETECT_APP_EVAL"))%></span><%
	Case rsAgreeDetect
%><span style="color:dimgray">导师已同意您的论文进行检测，请在教务员完成检测后上传送审论文。</span><%
	Case rsDetected
%><span style="color:dimgray">您的论文已完成检测，<a class="resc" href="fetchfile.asp?tid=<%=thesisID%>&type=12" target="_blank">点此下载检测报告</a>。同时，请在规定时间内上传送审论文。<br/>检测结果摘要：经图书馆检测，学位论文文字复制比为&nbsp;<%=toNumber(rs("REPRODUCTION_RATIO"))%>%。</span><%
	Case rsReviewThesisUploaded
%><span style="color:dimgray">在导师同意送审前，您可以重复上传送审论文文件；导师仅能看到您最新上传的论文。</span><%
	Case rsNotAgreeReview
%><span style="color:dimgray">导师不同意您的论文送审申请，请修改论文并重新上传。<br/>送审意见：<%=toPlainString(rs("REVIEW_APP_EVAL"))%></span><%
	Case rsAgreeReview
%><span style="color:dimgray">导师已于&nbsp;<%=toDateTime(rs("SUBMIT_REVIEW_TIME"),1)&" "&toDateTime(rs("SUBMIT_REVIEW_TIME"),4)%>&nbsp;同意您的论文送审申请。<br/>送审意见：<%=toPlainString(rs("REVIEW_APP_EVAL"))%></span><%
	Case rsMatchExpert
...
%><span style="color:dimgray">您的答辩论文未获导师<%=rs("TUTOR_NAME")%>审核通过，请修改后重新上传论文。审核意见如下：<br/>&emsp;&emsp;<%=toPlainString(rs("TUTOR_MODIFY_EVAL"))%></span><%
	Case rsModifyPassed
